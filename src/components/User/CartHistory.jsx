...
const fetchCarts = async (userId) => {
    ...
    return carts.filter(cart => cart.userId === userId); // Filter carts for the current user
};
...
const CartHistory = () => {
    ...
    const handleOrderClick = async (order) => {
        ...
        try {
            ...
            const details = await Promise.all(productIds.map(id => fetchProductDetails(id)));
    ...
    if (error) {
        return <Alert variant="danger" aria-live="assertive">{error.message}</Alert>;
    }
